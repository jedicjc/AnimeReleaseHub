...
export function AnimeGrid({ anime }: Props) {
  ...
  const filteredAnime = useMemo(() => {
    ...
    return anime.filter((item) =>
      `${getAnimeDisplayTitle(item)} ${item.japanese_title ?? ""}`
        .toLowerCase()
        .includes(search)
    );
  }, [anime, query]);
              ...
              >
                ...
                <div className="p-6">
                  ...
                  {item.trend_score !== null && item.trend_score !== undefined && (
                    <div className="mt-2 inline-flex rounded-full border border-pink-300/30 bg-pink-500/20 px-3 py-1 text-xs font-bold text-pink-200">
                      {getTrendBadge(item.trend_score)} · {item.trend_score}
                    </div>
                  )}
